...
def raw_results_retrieval(args, model_config, m, tx_tuple_list):
    ...
    if model_config == 0 or model_config == 1:
        ...
        lct = np.round(1 - (np.sum(timeseries_results[11]) / (np.sum(baseline_demand_hourly_mw[0:T]) +
                                                           ...
                                                           - np.sum(timeseries_results[10]))), 2)
...
def clcpa_results_compiling(args):

    columns = get_processed_columns()
    main_dir = os.path.join(args.results_dir, 'clcpa_results')
    compile_file = os.path.join(main_dir, 'processed_results_compiled.xlsx')

    lct_dirs = ['60%', '70%', '80%', '90%', '95%', '98%', '100%']
    compiled_results = pd.DataFrame(columns=columns)

    for dir in lct_dirs:
        try:
            results_df = pd.read_excel(glob.glob(os.path.join(main_dir, dir) +
                                                      '/processed_results_export.xlsx')[0], index_col= 0)
            compiled_results = compiled_results.append(results_df)

        except Exception as e:
            print('No data for LCT: {}'.format(dir))

    compiled_results.to_excel(compile_file)
